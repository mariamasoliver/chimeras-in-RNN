...
for i in range(nt):
    ...
    xhat = np.dot(np.transpose(sparse_phi),r[s2])
    ...
    if (i > imin):
        if (i < imax):
            if (i%step == 0):
                ...
                sparse_phi = sparse_phi - np.outer(np.dot(Pinv,r[s2]),e)
...
phi[s2,:] = sparse_phi
omega_learned = np.dot(sparse_eta,np.transpose(phi))
...
np.savetxt('phi_sparse_seed_'+str(llavor)+'_N_'+str(N)+str(c)+'_Q_'+str(Q)+'.txt',sparse_phi)
